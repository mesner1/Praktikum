...
public class Zapis_dopolniloDAO {
	 private Zapis_dopolniloDAO() {
		...
		public Zapis_dopolnilo najdiZapis_dopolnilo(int id) throws Exception {
			...
			try {
				conn=ds.getConnection();
				PreparedStatement ps = conn.prepareStatement("select * from zapis_dopolnilo where id=?",PreparedStatement.RETURN_GENERATED_KEYS);
				ps.setInt(1, id);
		...
		public void shraniZapis_dopolnilo(Zapis_dopolnilo o) throws Exception {
			...
			try {
				conn=ds.getConnection();
				if(o==null) return;
					PreparedStatement ps = conn.prepareStatement("insert into zapis_dopolnilo(dopolnilo_id , zapis_id, kolicina) values (?,?,?)",PreparedStatement.RETURN_GENERATED_KEYS);
		...
		public List<Zapis_dopolnilo> vrniVse() throws Exception {
			...
			try {
				...
				while (rs.next()) {
					Zapis_dopolnilo o = new Zapis_dopolnilo(rs.getInt("dopolnilo_id"), rs.getInt("zapis_id"), rs.getInt("kolicina"));
